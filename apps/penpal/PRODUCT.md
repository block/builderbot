...
- <a id="P-PENPAL-FAVORITE-ACTIONS"></a>**P-PENPAL-FAVORITE-ACTIONS**: Add to Favorites / Remove from Favorites is available from right-click context menus on file rows across the project sidebar, including detected sources, the Favorites section itself, and the project In Review section. Directory rows in project trees also expose add/remove favorites from their context menu. No inline star buttons are shown next to file or directory rows.
