...
if [ "$(uname -s)" != "Darwin" ]; then
    echo "Error: staged installer currently supports macOS only." >&2
    exit 1
fi
...
REPO_URL="${STAGED_REPO_URL:-${MARK_REPO_URL:-https://github.com/block/builderbot.git}}"
REPO_REF="${STAGED_REPO_REF:-${MARK_REPO_REF:-main}}"
SOURCE_DIR="${STAGED_SOURCE_DIR:-${MARK_SOURCE_DIR:-}}"
INSTALL_DIR="${STAGED_INSTALL_DIR:-${MARK_INSTALL_DIR:-/Applications}}"
CLI_DIR="${STAGED_CLI_DIR:-${MARK_CLI_DIR:-/usr/local/bin}}"
WORK_DIR="${STAGED_WORK_DIR:-${MARK_WORK_DIR:-}}"
KEEP_WORK_DIR="${STAGED_KEEP_WORK_DIR:-${MARK_KEEP_WORK_DIR:-0}}"

TEMP_DIR=""
if [ -z "$SOURCE_DIR" ]; then
    if [ -n "$WORK_DIR" ]; then
        mkdir -p "$WORK_DIR"
        TEMP_DIR="$(cd "$WORK_DIR" && pwd)/builderbot-staged-install"
        rm -rf "$TEMP_DIR"
    else
        TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/builderbot-staged-install.XXXXXX")"
    fi
...
if [ ! -f "$SOURCE_DIR/scripts/staged" ]; then
    echo "Error: could not find staged launcher script at $SOURCE_DIR/scripts/staged" >&2
    exit 1
fi

if [ ! -f "$SOURCE_DIR/apps/staged/justfile" ]; then
    echo "Error: could not find Staged app at $SOURCE_DIR/apps/staged" >&2
    exit 1
fi
...
APP_SOURCE="$SOURCE_DIR/apps/staged/src-tauri/target/release/bundle/macos/Staged.app"
CLI_SOURCE="$SOURCE_DIR/scripts/staged"
APP_DEST="$INSTALL_DIR/Staged.app"
CLI_DEST="$CLI_DIR/staged"

if [ ! -d "$APP_SOURCE" ]; then
    echo "Error: Staged.app was not produced at $APP_SOURCE" >&2
    exit 1
fi
...
printf 'Run `%s` to launch Staged from your current directory.\n' "$(basename "$CLI_DEST")"
