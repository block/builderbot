...
impl AgentDriver for AcpDriver {
    async fn run(
        ...
        let handler = Arc::new(AcpNotificationHandler::new(Arc::clone(writer)));
...
impl AcpNotificationHandler {
    fn new(writer: Arc<dyn MessageWriter>) -> Self {
        Self { writer }
    }
}
